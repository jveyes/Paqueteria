...
class SMSService:
    ...
    def _decode_jwt_payload(self, token: str) -> Optional[Dict]:
        """Decodificar el payload de un JWT para obtener la expiración"""
        try:
            parts = token.split('.')
            if len(parts) != 3:
                return None
    ...
    def _is_token_expired(self, token: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error verificando expiración del token: {e}")
            return True
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    if self._auth_token:
                        payload = self._decode_jwt_payload(self._auth_token)
                        if payload and payload.get('exp'):
                            exp_time = datetime.fromtimestamp(payload['exp'])
                            time_left = exp_time - self._token_created_at
                            logger.info(f"Autenticación LIWA.co exitosa - Token válido por {time_left}")
                        else:
    ...
    def _format_phone_number(self, phone: str) -> str:
        ...
        Args:
            phone (str): Número de 10 dígitos (ej: 3001234567) o 12 dígitos con código país (ej: 573001234567)
        ...
        # Si ya tiene código de país (12 dígitos), usarlo tal como está
        ...
        # Si no tiene 10 ni 12 dígitos, es inválido
        raise ValueError(f"El número de teléfono debe tener 10 dígitos (ej: 3001234567) o 12 dígitos con código país (ej: 573001234567), recibido: {len(clean_phone)} dígitos")
    ...
    async def send_tracking_sms(self, phone: str, customer_name: str, tracking_code: str, guide_number: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Verificar si necesitamos autenticarnos o renovar el token
            if not self._auth_token or self._is_token_expired(self._auth_token):
                logger.info("Token no disponible o expirado, renovando autenticación...")
