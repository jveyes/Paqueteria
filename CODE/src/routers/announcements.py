    ...
    if search:
        query = query.filter(
            or_(
                ...
                PackageAnnouncement.customer_phone.ilike(f"%{search}%"),
...
async def send_sms_from_browser(
    ...
    try:
        ...
        if not customer_name or not customer_phone or not guide_number or not tracking_code:
            missing_fields = []
        ...
        if not customer_phone.isdigit() or len(customer_phone) < 10:
            logger.error(f"Formato de teléfono inválido: {customer_phone}")
            return {
                "success": False,
        ...
        if sms_result.get("success"):
            logger.info(f"✅ SMS enviado exitosamente desde navegador a {customer_phone}")
            return {
                "success": True,
...
async def get_tracking_info(
    ...
    return {
        ...
        "package": {
            ...
            "customer_phone": package.customer_phone if package else announcement.customer_phone,
...
async def search_package_by_guide_number(
    ...
    # Si la query original coincide exactamente con el tracking_code, es consulta por código de tracking
    is_tracking_code_query = (announcement.tracking_code and 
                             query.upper() == announcement.tracking_code.upper())
    
    # Verificar si ya existen consultas previas para este tracking code
    existing_inquiries = []
    has_existing_email = False
    if is_tracking_code_query and announcement.tracking_code:
        logging.info(f"🔍 Verificando consultas existentes para tracking code: {announcement.tracking_code}")
        existing_inquiries = db.query(Message).filter(
            and_(
                Message.package_tracking_code == announcement.tracking_code,
                Message.message_type == MessageType.CUSTOMER_INQUIRY
            )
        ).all()
        
        logging.info(f"📊 Consultas encontradas: {len(existing_inquiries)}")
        
        # Verificar si alguna consulta tiene email
        has_existing_email = any(inquiry.customer_email is not None and inquiry.customer_email.strip() != '' for inquiry in existing_inquiries)
        logging.info(f"📧 Tiene email existente: {has_existing_email}")
        
        # Log de emails encontrados
        for inquiry in existing_inquiries:
            logging.info(f"📧 Email encontrado: {inquiry.customer_email}")
    else:
        logging.info(f"❌ No es consulta por tracking code o no hay tracking_code. is_tracking_code_query: {is_tracking_code_query}, tracking_code: {announcement.tracking_code if announcement else 'None'}")
    ...
    return {
        ...
        "search_query": query,
        "query_type": {
            "is_tracking_code_query": is_tracking_code_query,
            "should_show_history": is_tracking_code_query,
            "should_show_inquiry_form": is_tracking_code_query
        },
        "inquiry_info": {
            "has_existing_inquiries": len(existing_inquiries) > 0,
            "has_existing_email": has_existing_email,
            "inquiry_count": len(existing_inquiries),
            "latest_inquiry_date": existing_inquiries[-1].created_at.isoformat() if existing_inquiries else None
        } if is_tracking_code_query else None
    }
    ...
    logging.info(f"🔧 Respuesta final - is_tracking_code_query: {is_tracking_code_query}, inquiry_info: {inquiry_info_final}")
    logging.info("🔥 HOT RELOAD TEST - Cambio en Python detectado! ✅ FUNCIONANDO CON RDS!")
